/* Secure Shared Library (secure_app.js) */
...
// HTML escaping helper for sanitization
...
function logSimulationEvent(message) {
  ...
  // Ensure the logged message has its values sanitized/escaped
  state.logs.unshift({ timestamp, message: escapeHTML(message) });
...
function addEmailToWhitelist(email, name) {
  ...
  // Sanitize name and email before storing (Defense-in-Depth)
  const sanitizedName = escapeHTML(name.trim());
  const sanitizedEmail = escapeHTML(emailNorm);
  ...
  saveAppState(state);
  logSimulationEvent(`Whitelisted participant: ${sanitizedName} (${sanitizedEmail})`);
  return true;
}
